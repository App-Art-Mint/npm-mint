 ...
 * Panel settings
 */
export interface MintPanelSettings {
	id: string;
	wrapperId: string;
	title: string;
	from: EMintSide;
...
export class MintPanel extends MintAttachesEvents {
	 ...
	 */
	constructor(settings?: Partial<MintPanelSettings>) {
		super();
	...
	attachEvents(): void {
		...
		const toggleButtons = this.el.panel?.querySelectorAll<HTMLElement>(MintSelectors.controls(this.settings.wrapperId));
	...
	closeOtherPanels(): void {
		...
		const toggleBtn = document.querySelector<HTMLElement>(openPanelSelector);
		toggleBtn?.click();
	}
